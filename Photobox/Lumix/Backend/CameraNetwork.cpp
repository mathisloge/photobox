...
QString camMode()
{
    return "http://192.168.54.1/cam.cgi?mode=%1"_L1;
}
...
namespace pbox
{
CameraNetwork::CameraNetwork(QObject *parent)
    : QObject{parent}
{
    network_manager_.setAutoDeleteReplies(true);
    network_manager_.setTransferTimeout(std::chrono::seconds{5});

    connect(&network_manager_, &QNetworkAccessManager::finished, this, &CameraNetwork::parseNetworkReply);

    heartbeat_timer_.setInterval(std::chrono::seconds{5});
    connect(&heartbeat_timer_, &QTimer::timeout, this, &CameraNetwork::requestCameraState);
    heartbeat_timer_.start();

    lcd_keepalive_timer_.setInterval(std::chrono::seconds{45});
    connect(&lcd_keepalive_timer_, &QTimer::timeout, this, &CameraNetwork::requestLcdOn);
...
const CameraState &CameraNetwork::state() const
{
